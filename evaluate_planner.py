...
if __name__ == '__main__':
    ...
    # human_data = np.load("D:\\Projects\\TF2_ML\\openai.gym.human\\all_data.npy", allow_pickle=True)
    ...
    #             _, _, state_post = loadedModel(initial_states, a_prev, o_cur, training=False)
    #         else:
    #             if tf.squeeze(o_cur) == 0:
    #                 state_post_end[epi, :] = state_post.numpy().squeeze()
    #                 break
    #             else:
    #                 _, _, state_post = loadedModel(state_post, a_prev, o_cur, training=False)
    
    # # save the mean and std of the human goal state
    # end_state_mean = np.mean(state_post_end[:-1], axis=0)
    # end_state_std = np.std(state_post_end[:-1], axis=0)
