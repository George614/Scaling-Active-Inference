...
if __name__ == '__main__':
    ...
    prior_name = "zoo_data_old_gnll"  # "zoo_data"  #
    ...
    plot_rewards = False
    # model weights averaging during testing
    test_weight_avg = args.test_weight_avg
    ...
    epsilon_by_frame = Exponential_schedule(
        epsilon_start, epsilon_final, epsilon_decay)
    # gamma linear schedule for VAE regularization
    gamma_start = 0.01
    gamma_final = 0.99
    gamma_ep_duration = 200
    gamma_by_episode = Linear_schedule(
        gamma_start, gamma_final, gamma_ep_duration)
    ...
    model_save_path = os.path.join(
        model_save_path, now.strftime("%b-%d-%Y_%H-%M-%S"))
    ...
    if args.vae_optimizer == "AdamW":
        opt = tfa.optimizers.AdamW(
            learning_rate=args.vae_learning_rate, weight_decay=args.vae_weight_decay, epsilon=1e-5)
    else:
    ...
    if use_per_buffer:
        per_buffer = NaivePrioritizedBuffer(
            buffer_size * 2, prob_alpha=prob_alpha)
    else:
    ...
    for i in range(min(len(all_data), buffer_size)):
        o_t, action, reward, o_tp1, done = all_data[i, :obv_size], all_data[i,
                                                                            obv_size], all_data[i, obv_size+1], all_data[i, obv_size+2:-1], all_data[i, -1]
        if use_per_buffer:
    ...
    for ep_idx in range(num_episodes):  # training using episode as cycle
        ...
        while not done:
            ...
            if use_per_buffer:
                ...
                [batch_obv, batch_action, batch_reward, batch_next_obv,
                    batch_done, batch_indices, batch_weights] = batch_data
                batch_action = tf.one_hot(batch_action, depth=args.a_width)
                grads_efe, grads_model, loss_efe, loss_model, loss_l2, R_ti, R_te, efe_t, efe_target, priorities = pplModel.train_step(
                    batch_obv, batch_next_obv, batch_action, batch_done, batch_weights)
                ...
                if keep_expert_batch:
                    ...
                    if total_samples / len(replay_buffer) < batch_size:
                        ...
                        n_replay_samples = np.floor(
                            batch_size * len(replay_buffer) / total_samples)
                    ...
                    # else:
                        ...
                        expert_batch = expert_buffer.sample(
                            int(n_expert_samples))
                        replay_batch = replay_buffer.sample(
                            int(n_replay_samples))
                        batch_data = [np.concatenate((expert_sample, replay_sample))
                                      for expert_sample, replay_sample in zip(expert_batch, replay_batch)]
                    else:
                ...
                [batch_obv, batch_action, batch_reward,
                    batch_next_obv, batch_done] = batch_data

                batch_action = tf.one_hot(batch_action, depth=args.a_width)
                grads_efe, grads_model, loss_efe, loss_model, loss_l2, R_ti, R_te, efe_t, efe_target = pplModel.train_step(
                    batch_obv, batch_next_obv, batch_action, batch_done)
            ...
            weights_maxes = [tf.math.reduce_max(
                var) for var in pplModel.trainable_variables]
            weights_mins = [tf.math.reduce_min(
                var) for var in pplModel.trainable_variables]
            weights_max = tf.math.reduce_max(weights_maxes)
            weights_min = tf.math.reduce_min(weights_mins)

            grads_model_maxes = [tf.math.reduce_max(
                grad) for grad in grads_model if grad is not None]
            grads_model_mins = [tf.math.reduce_min(
                grad) for grad in grads_model if grad is not None]
            grads_model_max = tf.math.reduce_max(grads_model_maxes)
            grads_model_min = tf.math.reduce_min(grads_model_mins)

            grads_efe_maxes = [tf.math.reduce_max(
                grad) for grad in grads_efe if grad is not None]
            grads_efe_mins = [tf.math.reduce_min(
                grad) for grad in grads_efe if grad is not None]
            ...
            if frame_idx % log_interval == 0:
                ...
                tf.summary.scalar('grads_model_max',
                                  grads_model_max, step=frame_idx)
                tf.summary.scalar('grads_model_min',
                                  grads_model_min, step=frame_idx)
                tf.summary.scalar(
                    'grads_efe_max', grads_efe_max, step=frame_idx)
                tf.summary.scalar(
                    'grads_efe_min', grads_efe_min, step=frame_idx)
                tf.summary.scalar(
                    'R_ti_max', tf.math.reduce_max(R_ti), step=frame_idx)
                tf.summary.scalar(
                    'R_ti_min', tf.math.reduce_min(R_ti), step=frame_idx)
                tf.summary.scalar(
                    'R_te_max', tf.math.reduce_max(R_te), step=frame_idx)
                tf.summary.scalar(
                    'R_te_min', tf.math.reduce_min(R_te), step=frame_idx)
                tf.summary.scalar(
                    'EFE_old_max', tf.math.reduce_max(efe_t), step=frame_idx)
                tf.summary.scalar(
                    'EFE_old_min', tf.math.reduce_min(efe_t), step=frame_idx)
                tf.summary.scalar('EFE_new_max', tf.math.reduce_max(
                    efe_target), step=frame_idx)
                tf.summary.scalar('EFE_new_min', tf.math.reduce_min(
                    efe_target), step=frame_idx)
            ...
            if frame_idx % 200 == 0:
                print("frame {}, loss_model {:.3f}, loss_efe {:.3f}".format(
                    frame_idx, loss_model.numpy(), loss_efe.numpy()))
        ...
        if test_weight_avg == 'ema':
            ...
            if latest_count >= average_after:
                ...
                for i in range(latest_count, latest_count-winSize, -1):
                    ckpt_restored.restore(os.path.join(
                        model_save_path, "ckpt-{}".format(i)))
        ...
        # remove effect from checkpoint restoration
        test_Model.epsilon.assign(0.0)
        ...
        print("episode {}, mean reward {:.3f}, std reward {:.3f}".format(
            ep_idx+1, mean_reward, std_reward))
        ...
        if picky_swa and mean_reward > picky_reward:
            ...
            print("episode {}, swa reward {}".format(
                ep_idx+1, mean_test_reward))
    ...
    if crash:
        ...
        if plot_rewards:
            ...
            fig, ax = plt.subplots()
            ax.plot(np.arange(len(mean_ep_reward)), mean_ep_reward,
                    alpha=0.7, color='red', label='mean', linewidth=0.5)
            ax.fill_between(np.arange(len(mean_ep_reward)), np.clip(
                mean_ep_reward - std_ep_reward, -200, None), mean_ep_reward + std_ep_reward, color='#888888', alpha=0.4)
